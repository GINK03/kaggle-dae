...
print('load np')
train = np.vstack( [np.load(fn) for fn in sorted(glob.glob('vars/flatten_adam_train_*_*.npy')) ] )
train = np.hstack( [train, pd.read_csv('../input/train.csv').drop(['id', 'target'], axis=1).values] )
print('train shape', train.shape)
test  = np.vstack( [np.load(fn) for fn in sorted(glob.glob('vars/flatten_adam_test_*_*.npy')) ] )
